...
This task is a modification of task 8 for a specific application that is to capture the visual depiction of the Social Distancing aspects; an exercise that is being prescribed to all human beings during the Pandemic caused by the novel Covid-19 corona virus. The modifications made to Task 8 to achieve Task 9 is done by increasing the number of nodes where essentially the nodes represent people and this time the nodes are kept a distance of more than 6 units away from each other with the assumption that 1 unit maybe thought of as 1 feet, which was the condition kept to find the average number of steps. Our model can be used to calculate the probabilities of n number of nodes that are within 6 units of distance of contracting the virus. We carried out this task on 3 and 4 number of nodes and came up with the conclusion that as the number of nodes increase the probability of contracting the virus increases becuase the greater the number of nodes, the more closer they are likely to come towards each other in a confined space
